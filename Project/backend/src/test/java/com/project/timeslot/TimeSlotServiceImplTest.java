...
class TimeSlotServiceImplTest {
    ...
    void addTimeSlot_같은시간_예외() {
        ...
        try {
            ...
            assertThat(e.getMessage()).contains("시작 시간은 종료 시간보다 이전이어야 합니다.");
        }
    }
    ...
    void updateTimeSlot_성공() {
        ...
        // then
        TimeSlot updated = timeSlotRepository.findById(saved.getId()).get();
    ...
    void deleteTimeSlot_성공() {
        ...
        // then
        Optional<TimeSlot> deleted = timeSlotRepository.findById(id);
   ...
   // @Test
    ...
    void updateTimeSlot_예외() {
        ...
        // then
        assertThatThrownBy(() ->
                timeSlotService.updateTimeSlot(999L, dto))
                .isInstanceOf(IllegalArgumentException.class)
