...
public interface VolunteerRepository extends JpaRepository<Volunteer, Long> {
    ...
    @Query("""
    		SELECT new com.project.volunteer.dto.VolunteerCountDto(
    		    ts.id,
    		    ts.label,
    		    COALESCE(SUM(r.reserveNumber), 0),  
    		    ts.capacity
    		)
    		FROM TimeSlot ts
    		LEFT JOIN Volunteer v ON ts.id = v.timeSlot.id
    		LEFT JOIN Reserve r ON v.reserve.reserveCode = r.reserveCode
    		WHERE v.volDate = :date OR v.volDate IS NULL
